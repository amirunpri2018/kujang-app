...
class Guard
{
     ...
     *
     * For security, System-Csrf will *always* reset the token if there is a validation error.
     *
    ...
    public function getFailureCallable ()
    {
        if ( is_null( $this->failureCallable ) ) {
            $this->failureCallable = function ( ServerRequestInterface $request, ResponseInterface $response, $next ) {
                $body = new \System\Http\Body( fopen( 'php://temp', 'r+' ) );
     ...
     *
     * @param bool $persistentTokenMode True to use the same token throughout the session (unless there is a validation
     *                                  error), false to get a new token with each request.
     *
     * @return $this
     */
